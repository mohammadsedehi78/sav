...
class Agent_model extends Crud_model {
    ...
    function get_details($options = array(),$type) {
		...
		if ($type=='broker') {
            $where = " AND $clients_table.broker=1 ";
        }else{
			$where = " AND $clients_table.broker=0 ";
		}
		...
		*/
        ...
        $sql = "SELECT $clients_table.*,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='sales' GROUP BY client_id) AS total_sales,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='badane' GROUP BY client_id) AS total_badane,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='fire' GROUP BY client_id) AS total_fire,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='life' GROUP BY client_id) AS total_life,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='motor' GROUP BY client_id) AS total_motor,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='darman' GROUP BY client_id) AS total_darman,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='travel' GROUP BY client_id) AS total_travel,(SELECT COUNT(id) FROM $projects_table WHERE deleted=0 and insure='electronic' GROUP BY client_id) AS total_electronic, contacts_table.primary_contact, contacts_table.primary_contact_id,  project_table.total_projects, IFNULL(invoice_details.invoice_value,0) AS invoice_value, IFNULL(invoice_details.payment_received,0) AS payment_received $select_custom_fieds
        FROM $clients_table
